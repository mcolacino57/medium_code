...
class DecisionTree:
    ...
    def setup_graph(self):
        ...
        vertex_weights = {
            2: 0.1,
            3: 0.0,  # Action vertices
            4: 0.1,
            5: 0.0,
            6: -0.05,  # Market change vertices
            7: 0.1,
            8: 0.0,
            9: -0.05,  # Market change vertices
        }
    ...
    def analyze_decision_choices(self):
        ...
        if more_ev < no_more_ev:
            print(
                f"Optimal choice: Take More Space (saves ${no_more_ev - more_ev:,.2f})"
            )
        else:
            print(
                f"Optimal choice: Take No More Space (saves ${more_ev - no_more_ev:,.2f})"
            )
    ...
    def calculate_evpi(self):
        ...
        for v_more, v_no_more in zip(more_vertices, no_more_vertices):
            ...
            ev_no_more = self.get_vertex_probability(v_no_more) * self.calculate_cost(
                v_no_more
            )
            best_ev = min(ev_more, ev_no_more)
            ev_perfect += best_ev
            print(
                f"{v_more:5d} | ${ev_more:11,.2f} | ${ev_no_more:9,.2f} | ${best_ev:7,.2f}"
            )
